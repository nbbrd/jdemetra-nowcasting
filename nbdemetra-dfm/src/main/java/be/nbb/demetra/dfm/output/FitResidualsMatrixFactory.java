 ...
 * @author Mats Maggi
 */
@ServiceProvider(service = ProcDocumentItemFactory.class, position = 200121)
public class FitResidualsMatrixFactory extends DfmResultsItemFactory {

    public FitResidualsMatrixFactory() {
        super(newId(), newItemUI());
    }

    private static Id newId() {
        return new LinearId("Estimation", "Fit", "Residuals", "Matrix");
    }
    ...
    private static ItemUI<IProcDocumentView<DfmDocument>, Optional<DfmResults>> newItemUI() {
        return new DefaultItemUI<IProcDocumentView<DfmDocument>, Optional<DfmResults>>() {
            ...
            public JComponent getView(IProcDocumentView<DfmDocument> host, Optional<DfmResults> information) {
                return new ResidualsMatrixView(information);
            }
        };
    }
}
