 ...
 * @author Mats Maggi
 */
@ServiceProvider(service = ProcDocumentItemFactory.class, position = 400400)
public class RMSEItemFactory extends DfmSimulationResultsItemFactory {
    public RMSEItemFactory() {
        super(newId(), newItemUI());
        setAsync(true);
    }

    private static Id newId() {
        return new LinearId("Forecasts Simulation", "RMSE Graph");
    }
    ...
    private static ItemUI<IProcDocumentView<DfmDocument>, Optional<DfmSimulation>> newItemUI() {
        return new DefaultItemUI<IProcDocumentView<DfmDocument>, Optional<DfmSimulation>>() {
            ...
            public JComponent getView(IProcDocumentView<DfmDocument> host, Optional<DfmSimulation> information) {
                RMSEGraphView result = new RMSEGraphView(host.getDocument());
