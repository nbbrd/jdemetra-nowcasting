 ...
 * @author Mats Maggi
 */
@ServiceProvider(service = ProcDocumentItemFactory.class, position = 400500)
public class RealTimePerspItemFactory extends DfmSimulationResultsItemFactory {
    public RealTimePerspItemFactory() {
        super(newId(), newItemUI());
        setAsync(true);
    }

    private static Id newId() {
        return new LinearId("Forecasts Simulation", "Real time perspective");
    }
    ...
    private static ItemUI<IProcDocumentView<DfmDocument>, Optional<DfmSimulation>> newItemUI() {
        return new DefaultItemUI<IProcDocumentView<DfmDocument>, Optional<DfmSimulation>>() {
            ...
            public JComponent getView(IProcDocumentView<DfmDocument> host, Optional<DfmSimulation> information) {
                RealTimePerspGraphView result = new RealTimePerspGraphView(host.getDocument());
