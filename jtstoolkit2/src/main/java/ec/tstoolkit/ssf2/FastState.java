...
public class FastState extends BaseOrdinaryState implements Cloneable{
    ...
    public FastState clone() {
        FastState state = (FastState) super.clone();
        state.L = L.deepClone();
        return state;
    }
    ...
    public FastState(final ISsf ssf, final State state, final int pos) {
        ...
        // K0 = TPZ' / var
        Utilities.ZM(ssf, pos, state.P.columns(), C);
        ssf.TX(pos, C);
